...
moneyInputOnes(e) {
  ...
  shareFun() {
    wx.navigateTo({
      url:
        "SitePage/site/dieselInfo/dieselInfo?stationId=" + this.data.stationId,
    });
  },
    ...
    success: (result) => {
      ...
      if (fullUrl.includes('wx.ejiablue.com') && fullUrl.includes('/site')) {
        try {
          ...
          // 跳转到本小程序的柴油信息页
          wx.redirectTo({
            url: `/SitePage/site/dieselInfo/dieselInfo?stationId=${stationId}${connection ? `&connection=${connection}` : ''}`
          });
        } catch (e) {
    ...
    wx.requestPayment({
      ...
      success: (res) => {
        if (res.errMsg === "requestPayment:ok") {
          ...
          // 可跳转到支付成功页
          wx.navigateTo({
            url: `/SitePage/paymentSuccess/paymentSuccess?TradeNo=${params.trade_no}&orderInfo=${JSON.stringify(objs)}`,
          });
        }
      },
      fail: (err) => {
